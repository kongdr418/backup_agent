...
class ContentGenerator:
    ...
    def _generate_short_video_script(self, knowledge: str) -> str:
        ...
        prompt = f"""
请将以下专业知识转化为一份高度专业、具备视觉/听觉协同的短视频脚本（时长60-90秒）。

原始知识内容：
{knowledge}

# 角色指令
你是一位拥有千万粉丝的教育类短视频制作人，擅长通过"视觉冲击+口语化表达+价值升华"的工业化模式传递知识。

# 脚本排版格式（严格按表格输出）
请直接输出一个 Markdown 表格，包含以下列：
| 序号/时长 | 画面内容与运镜建议 | 【口播旁白】(VO) | 音效/BGM建议 | 字幕/视觉特效 |
| :--- | :--- | :--- | :--- | :--- |

# 内容创作要求
1. **黄金3秒开场**：必须从一个反常识或痛点切入。
2. **三维教学逻辑**：
   - **知识层**：将复杂定义转化为"通俗解释"。
   - **能力层**：提供具体的"操作建议"或"应用场景"。
   - **素养层**：结尾处提炼一个【重点标注 ★】灵魂金句进行价值升华。
3. **语言风格**：
   - 旁白必须口语化，适合 TTS 朗读。
   - 关键金句使用 ★ 号标注，作为视觉大字。

# 约束条件
- 拒绝废话，每5-8秒必须有视觉或听觉的节奏切换点。
- 旁白台词需放入标记符【 】内，方便后续提取。
- 请直接返回脚本内容，不要任何说明性文字。
"""
...
"""
    ...
    def _extract_voiceover(self, script: str) -> str:
        """从多维脚本表格中精准提取纯净旁白"""
        prompt = f"""
请从以下短视频脚本表格中，提取出所有被【 】括号包裹的【口播旁白】内容。

要求：
1. 仅提取【 】内的文字，去除括号本身。
2. 彻底剔除"画面描述"、"音效建议"、"字幕"等所有非语音内容。
3. 按顺序连接成一段通顺的文字，不要分行。
4. 保持口语语感。
...
请直接输出纯净的旁白文本：
"""
        return self._call_llm(prompt, "你是一个专业的视频编导，擅长精准提取音频文本")
