...
class CourseKnowledgeIngestor:
    ...
    def _detect_doc_kind(self, filename: str, content_type: str) -> str:
        ...
        if extension == ".xlsx" or content_type == XLSX_MIME:
            return "schedule"
        if extension in (".docx", ".pdf", ".pptx", ".txt", ".md") or content_type in (
            DOCX_MIME, PDF_MIME, PPTX_MIME, TXT_MIME, MD_MIME,
        ):
            return "syllabus"
        raise ValueError(f"unsupported file type: {extension or content_type}")
    ...
    def _parse_payload(self, doc_kind: str, filename: str, binary: bytes) -> dict[str, Any]:
        ...
        if doc_kind == "syllabus":
            if extension == ".pptx":
                return self._parse_pptx(filename, binary)
            if extension == ".pdf":
                return self._parse_pdf(filename, binary)
            if extension in (".txt", ".md"):
                return self._parse_text(filename, binary)
    ...
    def _parse_pdf(self, filename: str, binary: bytes) -> dict[str, Any]:
        ...
        return self._build_syllabus_payload(filename, raw_lines)

    # ── TXT / Markdown parser ──

    def _parse_text(self, filename: str, binary: bytes) -> dict[str, Any]:
    ...
    def _parse_pptx(self, filename: str, binary: bytes) -> dict[str, Any]:
        from pptx import Presentation
        prs = Presentation(io.BytesIO(binary))
        raw_lines: list[str] = []
        for slide in prs.slides:
            lines_in_slide: list[str] = []
            for shape in slide.shapes:
                if shape.has_text_frame:
                    for para in shape.text_frame.paragraphs:
                        text = para.text.strip()
                        if text:
                            lines_in_slide.append(text)
            if lines_in_slide:
                raw_lines.append("")
                raw_lines.extend(lines_in_slide)
        return self._build_syllabus_payload(filename, raw_lines)

    # ── shared syllabus extraction logic ──

    def _build_syllabus_payload(self, filename: str, raw_lines: list[str]) -> dict[str, Any]:
        paragraphs = self._merge_label_value_lines(raw_lines)
        course_name = ""
        summary = ""
        modules: list[dict[str, Any]] = []
        lessons: list[dict[str, Any]] = []
        ...
        # Try markdown heading as course name fallback (first # line)
        md_heading_match = re.match(r"^#\s+(.+)$", paragraphs[0] if paragraphs else "")
        first_heading_text = md_heading_match.group(1).strip() if md_heading_match else ""

        for line in paragraphs:
            if not line:
                continue
            # Skip markdown heading if we already used it as course name
            if first_heading_text and line.strip().lstrip("#").strip() == first_heading_text:
                continue
            label, value = _split_label_value(line)
            normalized_label = _normalize_label(label)
            if not course_name and normalized_label in COURSE_NAME_LABELS and value:
            ...
            if normalized_label in KNOWLEDGE_POINT_LABELS and value:
                ...
                for point in points:
                    if point not in knowledge_points:
                        knowledge_points.append(point)
                continue
            # heuristic: treat "XX：YY" lines as potential module/lesson
            if label and value:
                norm_label = _normalize_label(label)
                if any(norm_label.startswith(p) for p in MODULE_PREFIXES):
                    modules.append({
                        "module_id": f"mod_{_course_hash(value)}",
                        "title": value,
                        "lessons": [],
                    })
                    continue
                if any(norm_label.startswith(p) for p in LESSON_PREFIXES):
                    lessons.append({
                        "lesson_id": f"lesson_{_course_hash(value)}",
                        "title": value,
                        "knowledge_points": [],
                    })
                    continue
        ...
        if not course_name:
            course_name = first_heading_text or _safe_stem(filename)
