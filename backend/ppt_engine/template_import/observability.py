...
crossing 5x the threshold emits ``template_import.slow_step``.
...
# Per-step soft-deadline thresholds (ms). Crossing 5x the threshold emits a
...
def slow_step_warning(
    ...
    *,
    ...
    """Emit ``template_import.slow_step`` when a step exceeds 5x threshold."""
    logger.warning(
...
def check_slow_step(import_id: str, step_id: str, duration_ms: int) -> None:
    """Emit a ``slow_step`` warning when ``duration_ms`` exceeds 5x threshold.
