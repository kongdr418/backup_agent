...
appends an LLMTraceEntry per call.
"""
...
def _summarize_manifest(manifest: dict[str, Any]) -> dict[str, Any]:
    if not manifest:
        return {}
    keys = (
        "template_id", "label", "slide_count", "canvas", "theme",
        "page_type_candidates", "page_selections", "common_assets", "content_area",
    )
...
def _summarize_review(review: ReviewDraft) -> dict[str, Any]:
    if not review:
        return {}
    keys = (
        "import_id", "label", "slide_count", "page_selections",
        "page_type_candidates", "assets", "preserve_texts",
        "placeholder_hints", "element_actions", "feedback_history",
    )
    summary: dict[str, Any] = {}
    for key in keys:
        if key in review:
...
class LLMClient:
    ...
    async def call_with_retry(self, payload: dict[str, Any]) -> LLMTemplateImportPlan:
        ...
        system_prompt = (
            ...
            "Return exactly one JSON object matching the LLMTemplateImportPlan schema. "
            "No markdown, no prose, no code fences. "
            "Every reason field MUST be human-readable text in the user's language."
        )
        user_prompt = (
            ...
            "or ENDING_MESSAGE. Do not invent any other placeholder names.\n\n"
    ...
    async def _call_model(self, messages: list[dict[str, str]]) -> str:
        ...
        try:
            from ppt_engine.llm.registry import create_provider  # type: ignore[import-not-found]
            from ppt_engine.llm.types import LLMMessage  # type: ignore[import-not-found]
        except ImportError as exc:
            raise LLMPlanError(
                "no model client",
                error_kind="llm",
                context={"reason": f"ppt_engine.llm unavailable: {exc!r}"},
            ) from exc
    ...
    def _record_trace(
        ...
        *,
        ...
        trace = review.setdefault("llm_trace", [])
        trace.append(entry)
        ...
        for name, text in mapping.items():
            try:
                placeholder_decisions.append(
                    LLMPlaceholderDecision(page_type=page_type, name=name, text=text)
                )
            except ValidationError:
                continue
